...
export class AlarmConfigComponent extends React.Component<Props, State> {
    ...
    render() {
        ...
        return (config ? <div className="alarm-config-component">
            <AlarmConfigItemComponent onClick={this.toggleLight.bind(this)} state={config.lightState} icon={LightBulb}
                                      label='Light effects'></AlarmConfigItemComponent>
            <AlarmConfigItemComponent onClick={this.toggleSignal.bind(this)} state={config.signalState} icon={VolumeUp}
